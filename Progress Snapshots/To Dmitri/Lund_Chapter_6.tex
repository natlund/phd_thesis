...
\begin{center}
\begin{tikzpicture}
\node at (4,5) {Top Boundary Condition: $\frac{\partial u}{\partial z} = 1 $};
...
\fill [color=yellow,domain=0:8,samples=200] plot (\x,{ (3.2 /6.2823) * sin( 6.2832 *\x / 3.2 r)} ) -- ++(0,-2.5) -| (0,0);
...
\node at (2.4,-0.8) {Low Slip};
\node at (4,-0.2) [align=center] {High\\ Slip};

\draw[<->] (3.2,-2.2) -- node[above] {$L$} ++(3.2,0);
